...
function createWorld() {
    ...
    for (let x = -50; x <= 50; x += 10) {
        for (let z = -50; z <= 50; z += 10) {
                objects[balloonCounter].position.set(x, 55, z);
                balloonCounter++;
    ...
    function loadTankGLTF() {
        let tankLoader = new THREE.GLTFLoader();

        tankLoader.load('./model/tankBodyV4.gltf', (gltf) => {
            tankMesh = gltf.scene;
            scene.add(tankMesh);
            tankMesh.position.x = -2.25;
            tankMesh.position.y = -2;
            tankMesh.position.z = 1.9;
            tankMesh.rotation.y = 0.60;
        });
    }



    function loadBalloonGLTF() {
        let balloonLoader = new THREE.GLTFLoader();

        balloonLoader.load('./model/BalloonModel.gltf', (gltf) => {
            balloonMesh = gltf.scene;
            console.log("balloonMesh: " + balloonMesh)
            return balloonMesh;
        });
    }
    ...
    function moveBallObjects() {
        if (GameOverVar == false) {
            ...
            for (moveBalls = 0; moveBalls < objects.length; moveBalls++) {
                ...
                objects[moveBalls].rotation.y -= 0.05;
            }
            if (objPos >= 55) {
                objPosOffest = -0.05;
            }
    ...
    function moveRocket() {
        if(LoadedTankRocket == true) {
            let rocketObject = scene.getObjectByName('tankRocket');
            //console.log("tankRocket: " + rocketObject)
            rocketObject.position.y += 0.75;
        }
    }
...
/**
 * pregame lobby creation
 */
    function PregameStart() {
        console.log("PreGame function")
        ...
        let ground = new THREE.Mesh(
            ...
            new THREE.MeshLambertMaterial({
                color: "grey",
                map: makeTexture("resources/sand_rock.jpg") //add this to resources so it works
            })
        );
        ...
        let barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.7, 9, 16), gunmat);
        barrel.position.y = 2.5;
        gun.add(barrel);
        gunbase = new THREE.Mesh(new THREE.CylinderGeometry(3.5, 3.5, 0.75, 32), gunmat);
        ...
        let playLoader = new THREE.GLTFLoader();

        playLoader.load('./model/playButton.gltf', (gltf) => {
            let playMesh = gltf.scene;
            playMesh.scale.set(5, 5, 5);
            playMesh.rotation.y = -90;
            playMesh.position.set(0, 20, 0);
            playMesh.name = "startButton";
            scene.add(playMesh);
        });

        let restartLoader = new THREE.GLTFLoader();

        restartLoader.load('./model/restartButton.gltf', (gltf) => {
            let restartMesh = gltf.scene;
            restartMesh.scale.set(5, 5, 5);
            restartMesh.rotation.y = -90;
            restartMesh.position.set(0, 150, 0);
            restartMesh.name = "restartButton";
            scene.add(restartMesh);
        });

        let rocketLoader = new THREE.GLTFLoader();

        rocketLoader.load('./model/tankRocketNCL.gltf', (gltf) => {
            let rocketMesh = gltf.scene;
            rocketMesh.position.set(0, 150, 0);
            rocketMesh.name = "tankRocket";
            scene.add(rocketMesh);
            LoadedTankRocket = true;
            //let rocketObject = scene.getObjectByName('tankRocket');
            //console.log("created tankRocket: " + rocketObject);
        });
    ...
    for (let x = -50; x <= 50; x += 10) {
        for (let z = -50; z <= 50; z += 10) {
            if(colorCount == 1){
                colorValue = "./model/BalloonModelBlue.gltf";
                //console.log("IF 1");
            ...
            if(colorCount == 2){
                //console.log("IF 2");
                colorValue = "./model/BalloonModelGreen.gltf";
                //console.log("colorCount: " + colorCount);
            }
            if(colorCount == 3){
                //console.log("IF 2");
                colorValue = "./model/BalloonModelYellow.gltf";
                //console.log("colorCount: " + colorCount);
            }
            if(colorCount == 4){
                //console.log("IF 2");
                colorValue = "./model/BalloonModelOrange.gltf";
                //console.log("colorCount: " + colorCount);
            }
            if(colorCount == 5){
                //console.log("IF 2");
                colorValue = "./model/BalloonModelRed.gltf";
                //console.log("colorCount: " + colorCount);
            }
            if(colorCount == 6){
                //console.log("IF 2");
                colorValue = "./model/BalloonModelPink.gltf";
                //console.log("colorCount: " + colorCount);
            }
            if(colorCount == 7){
                //console.log("IF 3");
                colorValue = "./model/BalloonModelPurple.gltf";
                colorCount = 0;
            ...
            balloonLoader.load(colorValue, (gltf) => {
...
/**
 * fire rocket from tank
 * every time I fire a rocket, it adds all the balloons again. WHY???
 */
function fireRocket() {
    let rocketObject = scene.getObjectByName('tankRocket');
    if(rocketObject.position.y >= 80) {
        rocketObject.position.set(0, 3.5, 0);
    }
}

/**
 * clear game and display restart
 */
function gameOver() {

    for (let removeObj = 0; removeObj < objects.length; removeObj++) {
        objects[removeObj].position.set(0, 150, 0);
    }
    GameOverVar = true;
    let restartObject = scene.getObjectByName('restartButton');
    restartObject.position.set(-6, 20, -10);

    window.addEventListener('click', restartInit);
 ...
 *  for some reason, the remove event listener init, is not removed
 */
function restartInit() {
    console.log("restart Init");
    timeVar = 10;
    GameOverVar = false;
    window.removeEventListener('click', restartInit, false);
    window.addEventListener('click', fireRocket);
    let restartObject = scene.getObjectByName('restartButton');
    restartObject.position.set(0, 150, 0);
    console.log("removing: " + restartObject);
...
/**
    ...
    function init() {
        ...
        window.addEventListener('click', fireRocket);
        let selectedObject = scene.getObjectByName('startButton');
        selectedObject.position.set(0, 150, 0);
